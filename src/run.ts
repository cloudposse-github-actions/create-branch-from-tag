...
export async function createBranch(
  ...
  core.debug(
    `Trying to get branch ${branch} from ${context.repo.owner}/${context.repo.repo}`
  );
  ...
  const branchResponse = await octokit.rest.repos
    ...
    .catch(async (error: RequestError) => {
      ...
      if (error.name === "HttpError" && error.status === 404) {
        ...
        const longSHA = childProcess
          .execSync("git rev-list -n 1 " + from)
          .toString()
          .trim();
        core.debug(
          `Creating branch ${branch} from ${from} with SHA ${longSHA}`
        );
